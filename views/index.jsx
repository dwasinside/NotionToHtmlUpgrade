    ...
    switch (type) {
      ...
      case 'text':
        ...
        return (
          <>
            {link ? (
              <a {...linkProps}>{wrapContentWithAnnotations(textContent)}</a>
            ) : (
...
const renderBlock = ({ block, params, level = 0 }) => {
  ...
  switch (type) {
    ...
    case 'video':
      ...
      function transformYouTubeLink(inputLink) {
        const youtubeWatchRegex = /youtube\.com\/watch\?v=([\w-]+)/;
        const youtubePlaylistRegex = /list=([\w-]+)&/;
        const youtubeIndexRegex = /index=(\d+)/;
        const youtubeLiveRegex = /youtube\.com\/live\/([\w-]+)/;

        let transformedLink = inputLink;

        if (youtubeWatchRegex.test(inputLink)) {
          const videoId = inputLink.match(youtubeWatchRegex)[1];
          transformedLink = `https://www.youtube.com/embed/${videoId}?feature=oembed`;
        } else if (youtubePlaylistRegex.test(inputLink)) {
          const playlistId = inputLink.match(youtubePlaylistRegex)[1];
          transformedLink = `https://www.youtube.com/embed/?list=${playlistId}`;
        } else if (youtubeIndexRegex.test(inputLink)) {
          const videoId = inputLink.match(youtubeIndexRegex)[1];
          transformedLink = `https://www.youtube.com/embed/${videoId}?feature=oembed`;
        } else if (youtubeLiveRegex.test(inputLink)) {
          const liveId = inputLink.match(youtubeLiveRegex)[1];
          transformedLink = `https://www.youtube.com/embed/${liveId}?feature=oembed`;
        }
      ...
      const parseYoutubeUrl = (url) => {
        ...
        else return transformYouTubeLink(url);
